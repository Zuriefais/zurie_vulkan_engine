...
pub use once_cell;
use zurie::engine::core::{debug, error, info, trace, warn};
...
impl log::Log for EngineLogger {
    ...
    fn log(&self, record: &log::Record) {
        if self.enabled(record.metadata()) {
            match record.level() {
                log::Level::Error => error(record.args().as_str().unwrap()),
                log::Level::Warn => warn(record.args().as_str().unwrap()),
                log::Level::Info => info(record.args().as_str().unwrap()),
                log::Level::Debug => debug(record.args().as_str().unwrap()),
                log::Level::Trace => trace(record.args().as_str().unwrap()),
            }
        }
    }
...
#[repr(u32)]
#[derive(PartialEq, Debug, Eq, Hash, Clone, Copy, TryFromPrimitive)]
pub enum KeyCode {
